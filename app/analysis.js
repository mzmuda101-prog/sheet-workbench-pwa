...
// Leniwe doładowanie ciężkich analiz (duration + aggregation ~76KB) — parse dopiero
// przy pierwszym użyciu panelu. Po dograniu funkcje z analysis-heavy.js nadpisują stuby.
let _analysisHeavyPromise = null;
function ensureAnalysisHeavy() {
  if (window.__analysisHeavyLoaded) return Promise.resolve();
  if (!_analysisHeavyPromise) {
    _analysisHeavyPromise = new Promise((resolve, reject) => {
      const s = document.createElement("script");
      s.src = `app/analysis-heavy.js?v=${APP_BUILD_VERSION}`;
      s.async = false;
      s.onload = () => { window.__analysisHeavyLoaded = true; resolve(); };
      s.onerror = () => { _analysisHeavyPromise = null; reject(new Error("analysis-heavy")); };
      document.head.appendChild(s);
    });
  }
  return _analysisHeavyPromise;
}

function renderDurationAnalysis() {
  if (deferAnalysis("duration")) return;
  if (!window.__analysisHeavyLoaded) {
    ensureAnalysisHeavy().then(() => renderDurationAnalysis()).catch(() => {});
  }
}
...
function renderAggregationWorkbench() {
  ...
  if (!window.__analysisHeavyLoaded) {
    ensureAnalysisHeavy().then(() => renderAggregationWorkbench()).catch(() => {});
  }
}
